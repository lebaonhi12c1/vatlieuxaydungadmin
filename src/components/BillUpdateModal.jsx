...
function BillUpdateModal({isOpen, handleUpdateDeliveryBill,handleClose,handleSetBillUpdate,value,init}) {
    return (
        isOpen && <div className='flex items-center justify-center fixed inset-0 bg-black/20 overflow-y-auto'>
           <div className='flex flex-col gap-4 bg-white p-4'>
                <div className='text-[24px] text-center'>Update DeliveryBill</div>
                ...
                <div  className='flex flex-col gap-4'>
                   <div className='flex flex-col gap-2'>
                        ...
                        <input defaultValue={init.date} type="date" onChange={e=>handleSetBillUpdate(bill=>({...bill,date: e.target.value}))} className='p-2 border border-slate-400 rounded-sm'  />
                   </div>
                </div>
                <div className='flex items-center w-fit self-end gap-4'>
                    <button className='py-1 px-2 bg-blue-500 text-white rounded-sm' onClick={()=>handleUpdateDeliveryBill(value)}>Apply</button>
